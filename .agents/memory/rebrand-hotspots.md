...
- **Public landing page** `artifacts/qr-course/src/pages/Landing.tsx` — hardcodes the curriculum (a flat `topics` list, a "One Unit, N Sections" feature card, and a "A one-unit course…" badge) entirely independent of the seeded course data. Signed-out users see this, so a units/sections-count change (or rename) must update it too; the authed Dashboard derives from `/course/overview` and updates itself, but this page does NOT.
